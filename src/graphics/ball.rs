use crate::{graphics::{blocks::{Block, Level}, common_graphic_structs::{Coords, Dimensions, HasBounds, Input}, paddle::Paddle}, logic::vertex::Vertex};
...
impl Ball {
    ...
    fn is_colliding_with<T:HasBounds>(&self, other: &T) -> bool {
       self.position.x > other.position().x && self.position.x < other.position().x + other.dimensions().width &&
       self.position.y > other.position().y && self.position.y < other.position().y + other.dimensions().height ||
       self.position.x < other.position().x && self.position.x > other.position().x - other.dimensions().width &&
       self.position.y < other.position().y && self.position.y > other.position().y - other.dimensions().height
    }
    pub fn update(&mut self, buffer: &wgpu::Buffer, level: &mut Level, queue: &wgpu::Queue, paddle: &Paddle){
        for block in &mut level.blocks {
            if !block.is_active { break } ;
